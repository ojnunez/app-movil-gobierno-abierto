...
document.addEventListener("deviceready", devicePlatform, false);
  ...
  $.fn.extend({
    ...
    socialShare: function(options) {
      ...
      function createContainer(){
        ...
        'waze' : { text: 'Waze', className: 'aWaze', url: "waze://?ll={la},{lo}&navigate=yes" }
        };
        var sites = options.social.split(',');
        var listItem = '';
        var url = new Array();
        for (var i = 0; i <= sites.length-1; i++) {
          siteSettings[ sites[i] ]['url'] = siteSettings[ sites[i] ]['url'].replace('{t}',encodeURIComponent(options.title)).replace('{u}',encodeURI(options.shareUrl)).replace('{d}',encodeURIComponent(options.description)).replace('{la}',encodeURIComponent(options.latitude)).replace('{lo}',encodeURIComponent(options.longitude));
          url[i] = siteSettings[ sites[i] ]['url'];
          listItem += '<li><a onclick="window.open(\''+url[i]+'\',\'_system\');return false;" href="#" rel="nofollow" class="'+siteSettings[ sites[i] ]['className'] +'"><span></span></a><span>'+siteSettings[ sites[i] ]['text'] +'</span></li>';
        };
        //href="'+siteSettings[ sites[i] ]['url'] +'"
